...
df2 <- df %>%
  ...
  filter(geo_description %in% geo_areas,
         ...
         str_detect(description, 'Estimate; Income in the past 12 months below poverty level: - Other family: - Male householder,'))
df1 <- df %>%
  # only keep selected NC counties, the state of NC, and US data
  filter(geo_description %in% geo_areas,
         # only need rows showing the percent below poverty level, don't need count data
         str_detect(description, 'Estimate; Income in the past 12 months below poverty level: - Other family: - Female householder,'))
df <-rbind(df1,df2)
rm(df1,df2)
UnitedStates<-filter(df, geo_descr=='United States')
```
