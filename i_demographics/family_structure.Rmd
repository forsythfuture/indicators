...
cv_function<-function(se,estimate){
  cv<-(se/estimate)*100
  return(cv)
}
##########################################################
...
estimate<-aa_married_above[,4]+aa_married_below[,4]
above=aa_married_above[,5]
below=aa_married_below[,5]
percentage<-estimate[,1]/aa_total[,4]
percentage<-rename(percentage, percentage = estimate)
moe_married<-moe_function(above,below)
moe_above<-moe_function(above)
moe<-moe_proportion(moe_married[,1],aa_total[,5], percentage[,1], aa_total[,4])
#rename(moe, moe = estimate)
se<-se_function(moe_married[,1])
cv<-cv_function(se,estimate[,1])
race<-rep('African American',each=11)
aa_married<-cbind(location,year,description,percentage,moe,se,cv,race)
###############################################
location<-rep("Forsyth County", each=11)
year<-aa_female_above[,2]
description<-rep("Female householder, no wife present: with related children of the householder under 18 years", each=11)
estimate<-aa_female_above[,4]+aa_female_below[,4]
percentage<-estimate[,1]/aa_total[,4]
percentage<-rename(percentage, percentage = estimate)
...
cv<-cv_function(se,estimate[,1])
race<-rep('African American',each=11)
aa_female<-cbind(location,year,description,percentage,moe,se,cv,race)
#########################################################################################################
location<-rep("Forsyth County", each=11)
