...
#########################################################################################################
percentage<-left_join(aa_male_above,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_male_above<-cbind(aa_male_above,percent)
#########################################################################################################
percentage<-left_join(aa_male_below,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_male_below<-cbind(aa_male_below,percent)
#########################################################################################################
percentage<-left_join(aa_female_below,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_female_below<-cbind(aa_female_below,percent)
#########################################################################################################
percentage<-left_join(aa_female_above,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_female_above<-cbind(aa_female_above,percent)
#########################################################################################################
percentage<-left_join(aa_married_above,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_married_above<-cbind(aa_married_above,percent)
#########################################################################################################
percentage<-left_join(aa_married_below,aa_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
aa_married_below<-cbind(aa_married_below,percent)
#########################################################################################################
...
#########################################################################################################
percentage<-left_join(white_male_above,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_male_above<-cbind(white_male_above,percent)
#########################################################################################################
percentage<-left_join(white_male_below,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_male_below<-cbind(white_male_below,percent)
#########################################################################################################
percentage<-left_join(white_female_below,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_female_below<-cbind(white_female_below,percent)
#########################################################################################################
percentage<-left_join(white_female_above,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_female_above<-cbind(white_female_above,percent)
#########################################################################################################
percentage<-left_join(white_married_above,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_married_above<-cbind(white_married_above,percent)
#########################################################################################################
percentage<-left_join(white_married_below,white_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
white_married_below<-cbind(white_married_below,percent)
#########################################################################################################
###############################################HISPANIC#############################################################################################################################################################
df <- read_csv('data/family_structure_all_years.csv') %>%
  filter(geo_description %in% geo_areas) %>%
...
#########################################################################################################
percentage<-left_join(hl_male_above,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_male_above<-cbind(hl_male_above,percent)
#########################################################################################################
percentage<-left_join(hl_male_below,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_male_below<-cbind(hl_male_below,percent)
#########################################################################################################
percentage<-left_join(hl_female_below,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_female_below<-cbind(hl_female_below,percent)
#########################################################################################################
percentage<-left_join(hl_female_above,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_female_above<-cbind(hl_female_above,percent)
#########################################################################################################
percentage<-left_join(hl_married_above,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_married_above<-cbind(hl_married_above,percent)
#########################################################################################################
percentage<-left_join(hl_married_below,hl_total, by="year", COPY=FALSE)
percent<-percentage[,4]/percentage[,11]
percent<-rename(percent, percentage = estimate.x)
hl_married_below<-cbind(hl_married_below,percent)
#########################################################################################################
```
