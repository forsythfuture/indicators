...
df <- df %>%
        mutate(var_name = (replace(df$var_name, df$var_name == "Total;", "Total"))) %>%
        # remove all of the percent estimate & percent moe columns from 2017 data that are not in any of the            other years 
...
pop_total <- df %>%
  ...
  select(description, var_name, estimate, moe, year) %>%
  select(-description)
...
df <- df %>%
  left_join(pop_total, by = c('var_name','year')) %>%
