#This is different from what Joanna has
#These functions have options to specify the demographic category of each measurement 
#which is passed to the end of the variable name

#It will use data before 2012 if you first download the AFF table and place it in the directory.
...
setwd('C:/Users/staff/Desktop/Data/R/R-code')
...
variable_list <- load_variables(2016, "acs1", cache = TRUE) 
...
import_acs <- function(acs_var,varname,survey = "acs1",year = 2011,
        ...
        if(file.exists(filename)){
                ...
                #verify that you have the right variable
                varlist <- load_variables(2016, survey, cache = TRUE)
                fullvarname <- varlist[varlist$name==paste0(acs_var,"E"),c('concept','label')]
                
                if(print_labels){
                        print(paste(varname_out,"created based on",label,"from",filename,"which in 2016 was",fullvarname[2]))
                }
...
acs_newvar <- function(df,acs_var,varname,survey = "acs1",year = 2017,get_NC=1,
                       counties=c("Forsyth","Guilford","Durham"),
                       states=c("NC","NC","NC"),
                       age1="00",age2="99",MF="A",race_eth="A"){
...
acs_subtract <- function(df,var1,var2,varname,suffix="0099AA17"){
...
for (year in c(2016,2015,2014,2013,2012,2011,2010,2009,2008,2007)){
#for (year in c(2006)){
