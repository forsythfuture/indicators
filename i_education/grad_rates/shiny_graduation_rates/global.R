...
plotly_plots <- function(df, input_type, plot_type) {
  ...
  # filter out schools, as we don't need to plot them
  df <- df %>%
    filter(geo_area != 'school')
  ...
  if (plot_type == 'line') {
    ...
    df %>%
      ...
      filter(geo_description == if (input_type != 'Total') 'Forsyth County, NC' else .$geo_description) %>%
      plot_ly(x = ~year, y = ~estimate, 
              color = if (input_type == 'Total') ~geo_description else ~subtype, 
...
ff_acs_zscore <- function(data_frame, success, trials, var_names = NULL) {
  ...
  # create vectors of graduates and totals, 
...
ff_acs_zscore_kable <- function(data_frame, estimate, se, var_names = NULL, table_name = 'Z-Scores') {
  ...
  data_frame %>%
    ...
    mutate_all(funs(cell_spec(., bold = ifelse(. < 0.05, T,F)))) %>%
