...
party <- df %>%
        ...
        summarize(count = n())%>%
        mutate(estimate = count/nrow(df)*100, type = 'party')%>%
...
write_csv(age_count,"data/age_count.csv")
...
tableau_data <- bind_rows(gender,ethnicity,race, party)%>%
...
tableau_data <- as.data.frame(tableau_data)
