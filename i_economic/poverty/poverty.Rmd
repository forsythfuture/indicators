...
total_data %>%
  datatable(filter='top', extensions='Buttons',
            colnames = c('Geographic area', 'Year', 'Poverty Rate', '95% MOE', 'St. Error', 'CV'),
...
ethnicity_data %>%
  datatable(filter='top', extensions='Buttons',
            colnames = c('Geographic area', 'Year', 'Ethnicity', 'Poverty Rate', '95% MOE', 'St. Error', 'CV'),
            options = list(scrollX = TRUE, scrollY = TRUE, dom = 'Bfrtip')) %>%
  # color cv numbers based on value
  formatStyle('cv', color = styleInterval(c(12, 30), c('black', 'blue', 'red')))
```
...
age_data %>%
  datatable(filter='top', extensions='Buttons', 
            colnames = c('Geographic area', 'Year', 'Age group', 'Poverty Rate', '95% MOE', 'St. Error', 'CV'),
            options = list(scrollX = TRUE, scrollY = TRUE, dom = 'Bfrtip')) %>%
  # color cv numbers based on value
    formatStyle('cv', color = styleInterval(c(12, 30), c('black', 'blue', 'red')))
```
...
if (write_to_excel == TRUE) {
  ...
  excel_data %>%
    ...
    # write to excel
    ff_write_to_excel(., 'poverty_rate.xlsx')
}
```